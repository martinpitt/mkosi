...
def test_venv_installation() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create venv
        run(["python3", "-m", "venv", fspath(venv)], cwd=REPO_ROOT)
        pip = venv / "bin/python3"

        # Upgrade pip, setuptools, wheel
        run([fspath(pip), "-m", "pip", "install", "--upgrade", "setuptools", "wheel", "pip"], cwd=REPO_ROOT)

        # Install mkosi
        run([fspath(pip), "-m", "pip", "install", "."], cwd=REPO_ROOT)

        # Test that mkosi works
        run([fspath(venv / "bin/mkosi"), "-h"], cwd=REPO_ROOT)
...
def test_editable_venv_installation() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        run([fspath(pip), "-m", "pip", "install", "--editable", "."], cwd=REPO_ROOT)
